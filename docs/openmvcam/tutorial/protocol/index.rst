...
built with DearPyGui or Qt, another cam on the other end of a UART,
or a service running on a workstation watching a fleet of cameras.
...
The :mod:`protocol` module on the cam side and the
:mod:`openmv-python <openmv.camera>` package on the host side answer
both, by letting a Python class on the cam expose a *channel* that a
Python class on the host can read from, write to, and react to events
on, all over a single USB or serial connection.

.. image:: figures/system-overview.svg
   :alt: A host PC connects to a cam over USB; the cam exposes three
         channels -- a frame channel for image data, a config channel
         for typed widget values, and the built-in stdout channel
         for prints -- and the host script reads or writes each.
   :align: center
...
tools that ship in the ``openmv-projects/tools/`` directory use
exactly the patterns shown here.
...
.. toctree::
   ...
   hello-world/cam-side.rst
   hello-world/host-side.rst
...
.. toctree::
   ...
   widgets/cbor-and-typed-values.rst
   widgets/building-a-gui.rst
