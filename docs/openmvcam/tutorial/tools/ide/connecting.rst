...
* *Serial Port* -- the port the connection is using.
  Click it to give the camera an IDE-local name, shown
  here in place of the port so identical cameras are
  tellable apart; the name is remembered per camera and
  is local to your IDE.
