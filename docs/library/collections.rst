...
the built-in :class:`list`, :class:`tuple`, :class:`dict` and :class:`set`: a double-ended
...
.. class:: deque(iterable: Iterable, maxlen: int, flags: int = 0)
    ...
    Deque objects support :class:`bool`, :func:`len`, iteration and subscript load and store.
    They also have the following methods:

    .. method:: deque.append(x: Any) -> None

        Add *x* to the right side of the deque.
        Raises :exc:`IndexError` if overflow checking is enabled and there is
    ...
    .. method:: deque.pop() -> Any
        ...
        Raises :exc:`IndexError` if no items are present.
...
.. class:: OrderedDict(*args: Any, **kwargs: Any)
    ...
    :class:`dict` type subclass which remembers and preserves the order of keys
    added. Accepts the same constructor forms as :class:`dict` (an iterable of
